...
def mixed_sup(nth = 0.15, scattering = 0.002, wcs1_amp = 0., wcs2_amp = 0.):
	...
	wcs1_mode = coherent_dm(N, wcs1_amp)
	...
	#now the second interference step
	#maximum visibility is attained for angle pi/2
	angle = np.pi/2

	if wcs2_amp == 0.:
		wcs2_amp = np.sqrt(n_mech)
...
pts2 = 4
p_blue = 0.002
wcs1_amp = np.linspace(0.0001, 0.02, pts1)
...
for jj in range(pts2):
	for ii in range(pts1):
		[n[jj, ii], v[jj, ii]] = mixed_sup(ntherm[jj], p_blue, np.sqrt(wcs1_amp[ii]), 0.)

	ax1.plot(wcs1_amp, v[jj, :])
	ax2.plot(wcs1_amp, np.sqrt(n[jj, :]))

np.savetxt('blue_n_fock.dat', n)
np.savetxt('blue_vis.dat', v)

ax2.set_xlabel('n_coherent')
ax1.set_ylabel('vis')
ax2.set_ylabel('n_mech')
plt.legend(['n_th = 0.00', '0.05', '0.15', '0.30' ])
plt.show()
